...
# sp = StanfordPOSTagger('../bin/stanford-postagger-full-2018-02-27/models/english-bidirectional-distsim.tagger',
#                        '../bin/stanford-ner-2018-02-27/stanford-ner-3.9.1.jar',
#                        encoding='utf-8')

class3 = '../bin/stanford-ner-2018-02-27/classifiers/english.all.3class.distsim.crf.ser.gz'
class7 = '../bin/stanford-ner-2018-02-27/classifiers/english.muc.7class.distsim.crf.ser.gz'
...
with open("ARC-Challenge-Test_qapredictions_dgem_default.jsonl", "r") as in_file, open("results/named_entities.txt", "w") as out_file:
    ...
    cleaned_data = [regex.sub('', token) for token in data]
    cleaned_data = [token for token in cleaned_data if token not in stop_words]
